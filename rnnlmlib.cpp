...
// put all the unique words into the vocabulary structure
// and then update the count of each words
// and create the hashmap from the word to the position of the word in the vocal structure
// the hashmap might be only used here
// they use bubble sort based on the count
...
void CRnnLM::saveWeights()      //saves current weights and unit activations
{
	...
	for (a=0; a<layer_in_size; a++) {
		neu_inb[a].ac=neu_in[a].ac;
		neu_inb[a].er=neu_in[a].er;
	}


	for (a=0; a<layer_su_size; a++) {
		neu_sub[a].ac=neu_su[a].ac;
		neu_sub[a].er=neu_su[a].er;
	}
	...
	for (b=0; b<layer_su_size; b++) for (a=0; a<layer_in_size; a++) {
...
void CRnnLM::initNet()
{
	...
	layer_su_size = factor_size;
	layer0_size = factor_size;
	...
	syn_uf = (struct synapse *)calloc(layer_su_size*layer0_size*domain_size, sizeof(struct synapse));
	...
	syn_d=(direct_t *)calloc((long long)direct_size, sizeof(direct_t)); //???????? direct_size = 2 * 1000000
	...
	neu_inb = (struct neuron *)calloc(layer_in_size, sizeof(struct neuron));
	neu_sub = (struct neuron *)calloc(layer_su_size, sizeof(struct neuron));
	neu0b=(struct neuron *)calloc(layer0_size, sizeof(struct neuron));
	neu1b=(struct neuron *)calloc(layer1_size, sizeof(struct neuron));
	neucb=(struct neuron *)calloc(layerc_size, sizeof(struct neuron));//layerc_size = 0, don't care
	neu1b2=(struct neuron *)calloc(layer1_size, sizeof(struct neuron));
	neu2b=(struct neuron *)calloc(layer2_size, sizeof(struct neuron));

	syn_uub = (struct synapse *)calloc(layer_in_size*layer_su_size, sizeof(struct synapse));
	syn_ufb = (struct synapse *)calloc(layer_su_size*layer0_size*domain_size, sizeof(struct synapse));
	...
	//initialization of all the neurons
	for(a=0;a<layer_in_size;a++){
		neu_in[a].ac=0;
		neu_in[a].er=0;
	}
	...
	//initialization of all the synapses
	...
	if (bptt>0) { // bptt = 4 in the script
		bptt_history=(int *)calloc((bptt+bptt_block+10), sizeof(int)); // bptt_block = 10 in the script (why add 10 more here?)
		for (a=0; a<bptt+bptt_block; a++) bptt_history[a]=-1;
		//
		bptt_hidden=(neuron *)calloc((bptt+bptt_block+1)*layer1_size, sizeof(neuron)); // why bptt+bptt_block+1?
	...
	} else {			// new classes //default is zero here
		for (i=0; i<vocab_size; i++) b+=vocab[i].cn;
...
{
	//Refer to the normal implementation below: it only uses the vocab part to get the values of neuron2
...
void CRnnLM::computeNet(int last_word, int word)
{
	...
	//propagate 0->1
	for (a=0; a<layer_su_size; a++) neu_su[a].ac=0;
	for (a=0; a<layer0_size; a++) 	neu0[a].ac=0;
	for (a=0; a<layer1_size; a++) 	neu1[a].ac=0;
	for (a=0; a<layerc_size; a++)   neuc[a].ac=0; //useless


	//for(int no_node = 0 ; no_node < layer_in_size;no_node++)//TODO modified later
	...
	for (b=0; b<layer_su_size; b++) {//use the last word to further update the hidden layer
			...
			if (a!=-1) neu_su[b].ac += neu_in[a].ac * syn_uu[a+b*layer_in_size].weight;
	}  //complete the activation till su layer
	...
	for (a=0; a<layer_su_size; a++) {
			...
			if (neu_su[a].ac<-50) neu_su[a].ac=-50;  //for numerical stability
			val=-neu_su[a].ac;
			neu_su[a].ac=1/(1+fasterexp(val));
	}


	for(int no_node = 0 ; no_node < layer0_size;no_node++)//TODO modified later
	...
	if (layerc_size>0) { // never never never use this one
...
void CRnnLM::learnNet(int last_word, int word)
{
	...
	for (c=0; c<class_cn[vocab[word].class_index]; c++) { //traverse all the word in that class where the word belongs to
		a=class_words[vocab[word].class_index][c];
	...
	///////////////////////////////////////////IMPLEMENTATION/////////////////////////////
	for (a=0; a<layer0_size; a++) neu0[a].er=0;
	for (a=0; a<layer_su_size; a++) neu_su[a].er=0;

	for (a=vocab_size; a<layer2_size; a++) { //update the class neurons
		neu2[a].er=(0-neu2[a].ac);
	}
	...
	else
	{
		...
		for (c=0; c<class_cn[vocab[word].class_index]; c++) {
			b=class_words[vocab[word].class_index][c];
			if ((counter%10)==0)	//regularization is done every 10. step //update the relevant synapse
		...
		for (b=vocab_size; b<layer2_size; b++) {
			if ((counter%10)==0) {	//regularization is done every 10. step
				...
				for (a=0; a<layer1_size; a++) syn1[a+c].weight+=alpha*neu2[b].er*neu1[a].ac;	//weight 1->2 update?? Should be 2->1 update?
			}
			c+=layer1_size;
		}

		//////////////////////////////IMPLEMENTATION////////////////////////////////////
		for (a=0; a<layer1_size; a++) neu1[a].er=neu1[a].er*neu1[a].ac*(1-neu1[a].ac);  //real errors for neuron1

		c=0;
		for (b=0; b<layer1_size; b++) {
			if ((counter%10)==0) {	//regularization is done every 10. step
				for (a=0; a<layer0_size; a++) syn0[a+c].weight+=alpha*neu1[b].er*neu0[a].ac - syn0[a+c].weight*beta2;	//weight 1->2 update??
			}
			else {
				for (a=0; a<layer0_size; a++) syn0[a+c].weight+=alpha*neu1[b].er*neu0[a].ac;	//weight 1->2 update?? Should be 2->1 update?
			}
			c+=layer0_size;
		}

		matrixXvector(neu0, neu1, syn0, layer0_size, 0, layer1_size, 0, layer0_size, 1);
		for (a=0; a<layer0_size; a++) neu0[a].er=neu0[a].er*neu0[a].ac*(1-neu0[a].ac);  //real errors for neuron1

		for(b=0;b<layer_su_size;b++){
			neu_su[b].er = neu0[b].er * syn_uf[b+tmp_d*(layer_su_size*layer0_size)].weight;
			if(counter%10==0)
		...
		for(int no_node = 0 ; no_node < layer_su_size;no_node++)//TODO modified later
			neu_su[no_node].er = neu0[no_node].er * syn_uf[no_node+tmp_d*(layer_su_size*layer0_size)].weight;
	}
	...
	if (bptt<=1) {		//bptt==1 -> normal BP
		...
		if (a!=-1) {
			if ((counter%10)==0)
				for (b=0; b<layer_su_size; b++) syn_uu[a+b*layer_in_size].weight+=alpha*neu_su[b].er*neu_in[a].ac - syn_uu[a+b*layer_in_size].weight*beta2;
			else
				for (b=0; b<layer_su_size; b++) syn_uu[a+b*layer_in_size].weight+=alpha*neu_su[b].er*neu_in[a].ac;
		}

		if ((counter%10)==0) {
			for (b=0; b<layer_su_size; b++) for (a=layer_in_size-layer1_size; a<layer_in_size; a++) syn_uu[a+b*layer_in_size].weight+=alpha*neu_su[b].er*neu_in[a].ac - syn_uu[a+b*layer_in_size].weight*beta2;
		}
		else {
			for (b=0; b<layer_su_size; b++) for (a=layer_in_size-layer1_size; a<layer_in_size; a++) syn_uu[a+b*layer_in_size].weight+=alpha*neu_su[b].er*neu_in[a].ac;
		}
	...
	else		//BPTT
	{
		...
		if (((counter%bptt_block)==0) || (independent && (word==0))) {
			for (step=0; step<bptt+bptt_block-2; step++) { // for this for loop you only deal with the context part
				for (a=0; a<layer1_size; a++) neu1[a].er=neu1[a].er*neu1[a].ac*(1-neu1[a].ac);    //error derivation at layer 1
				...
				//update the context part of neuron 0

				for (b=0; b<layer1_size; b++)
					for (a=layer0_size-layer1_size; a<layer0_size; a++) {
					//neu0[a].er += neu1[b].er * syn0[a+b*layer0_size].weight;
					bptt_syn0[a+b*layer0_size].weight+=alpha*neu1[b].er*neu0[a].ac; //update all the context part
				}
				...
				if (step<bptt+bptt_block-3)//goes for every step except the last step
					for (a=0; a<layer1_size; a++) {
			...
			for (b=0; b<layer1_size; b++) {		//copy temporary syn0
				if ((counter%10)==0) { // regularization for counter % 10 ==0
					for (a=layer0_size-layer1_size; a<layer0_size; a++) {
					...
					for (a=layer0_size-layer1_size; a<layer0_size; a++) {//update the context part
				...
				if ((counter%10)==0) {
					...
					for (step=0; step<bptt+bptt_block-2; step++) if (bptt_history[step]!=-1) {
						syn0[bptt_history[step]+b*layer0_size].weight+=bptt_syn0[bptt_history[step]+b*layer0_size].weight;//update the vocab part
						bptt_syn0[bptt_history[step]+b*layer0_size].weight=0;
					}
				}
			}
...
void CRnnLM::trainNet()
{
	...
	counter=train_cur_pos; // 0 by default
	//saveNet();
	while (iter < maxIter) {
		printf("Iter: %3d\tAlpha: %f\t   ", iter, alpha);
		fflush(stdout);

		if (bptt>0) for (a=0; a<bptt+bptt_block; a++) bptt_history[a]=0; //the size of bptt_history = bptt + bptt_block + 10
		for (a=0; a<MAX_NGRAM_ORDER; a++) history[a]=0; // MAX_NGRAM_ORDER = 20, the size of the history = MAX_NGRAM_ORDER
		...
		while (1) {
			...
			if (bptt>0) {		//shift memory needed for bptt to next time step
				for (a=bptt+bptt_block-1; a>0; a--) bptt_history[a]=bptt_history[a-1]; //contains the index of the word, the leftmost is the latest word
				bptt_history[0]=last_word;

				for (a=bptt+bptt_block-1; a>0; a--) for (b=0; b<layer1_size; b++) {//Should check later
